...
import { Printer, ArrowLeft, MailCheck } from "lucide-react";
...
export function ForgotPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(extractError(err, "No se pudo procesar la solicitud"));
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="flex min-h-screen items-center justify-center bg-secondary/40 px-4">
      <div className="w-full max-w-sm rounded-2xl border border-border bg-card p-8 shadow-card">
        ...
        {sent ? (
          ...
          <>
            <h1 className="mb-1 text-xl font-bold">Olvidé mi contraseña</h1>
            <p className="mb-6 text-sm text-muted-foreground">
